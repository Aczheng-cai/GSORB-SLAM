...
torch::Tensor filter_radii(torch::Tensor means3D,
    ...
    return RasterizeGaussiansfilterCUDA(
        means3D,
        scales,
        rotations,
  ...
  const int device_num)
{
  ...
  torch::Tensor out_color = torch::full({NUM_CHANNELS, H, W}, 0.0, float_opts).to(device);
  torch::Tensor radii = torch::full({P}, 0, means3D.options().dtype(torch::kInt32)).to(device);
  torch::Tensor out_depth = torch::full({1, H, W}, 0.0, float_opts).to(device);
